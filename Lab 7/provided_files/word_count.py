...
def hash_function_3(val):
    hash_sum = 0
    for i, j in enumerate(val):
        hash_sum += ord(j) * pow(31, i)
    return hash_sum
...
def main():
    ...
    if capacity < 0:
        hTable = HashMap(hash_function_2)
    else:
        hTable = HashMap(hash_function_2, capacity)
    filename = input("Enter filename: ")

    wordTable = word_count(hTable, filename)
    print(hTable.imbalance())

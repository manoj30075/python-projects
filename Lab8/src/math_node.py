...
CSCI-603 Parser Lab
Author: RIT CS
Author: Author: Manoj Kumar Reddy Palasamudram, Ashwath Halemane
...
class MathNode:
    ...
    def __init__(self, left, right, token):
        ...
        :return: None
        """
        if token not in pretee.PreTee.MATH_TOKENS:
    ...
    def evaluate(self):
        ...
        if self.token == pretee.PreTee.MULTIPLY_TOKEN:
            return self.left.evaluate() * self.right.evaluate()
        if self.token == pretee.PreTee.DIVIDE_TOKEN:
            if self.right.evaluate() == 0:
                raise runtime_error.RuntimeError("hello")
            return self.left.evaluate() // self.right.evaluate()
        if self.token == pretee.PreTee.ADD_TOKEN:
            return self.left.evaluate() + self.right.evaluate()
        if self.token == pretee.PreTee.SUBTRACT_TOKEN:
            return self.left.evaluate() - self.right.evaluate()

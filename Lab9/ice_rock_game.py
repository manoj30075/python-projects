"""
file: ice_rock_game.py
description: A game where user has to escape through a maze.
language: python3
...
def find_neighbors(row, column, matrix):
    """
    ...
    # Check if the current vertex is on the left edge
    temp = column + 1
    ...
    # Check if the current vertex is on the right edge
    while temp >= 0:
        if matrix[row][temp] == '*':
            if (temp + 1) != column:
                left = (row, temp + 1)
    ...
    # Check if the current vertex is on the top edge
    while temp < len(matrix):
        if matrix[temp][column] == '*':
            if temp - 1 != row:
                bottom = (temp - 1, column)
            break
    ...
    # Check if the current vertex is on the bottom edge
    while temp >= 0:
        if matrix[temp][column] == '*':
            if temp + 1 != row:
                top = (temp + 1, column)
...
def construct_path(out_vertex, ice_maze_graph):
    ...
    for vertex in ice_maze_graph.getVertices():
        if vertex == out_vertex:
            continue
        # Find the shortest path from the current vertex to the exit
        ...
        if path_to_exit.get(edges) is None:
            path_to_exit[edges] = [new_vertex]
        else:
            path_to_exit[edges].append(new_vertex)

    for key in sorted(path_to_exit.keys()):
        if key != 0:
            print(key, ':', path_to_exit[key])
    if 0 in path_to_exit:
        print("No path", ':', path_to_exit[0])
        ...
        for j in range(len(matrix[i])):
            if matrix[i][j] == '.':
                ...
                left, right, top, bottom = \
                    find_neighbors(i, j, matrix)
                if left:
                    maze_graph.addEdge((i, j), left)
                if right is None:
                    if (i, j) == out_vertex:
                        right = (out_vertex[0], out_vertex[1] + 1)
                        maze_graph.addEdge((i, j), right)
                elif right == out_vertex:
                    right = (out_vertex[0], out_vertex[1] + 1)
                    maze_graph.addEdge((i, j), right)
                else:
                    maze_graph.addEdge((i, j), right)
                if top:
                    maze_graph.addEdge((i, j), top)
                if bottom:
                    maze_graph.addEdge((i, j), bottom)
            else:
                if maze_graph.getVertex((i, j)) is None:
                    maze_graph.addVertex((i, j))
    return maze_graph
...
def main():
    ...
    construct_path((out_row, len(grid[1])), ice_maze_graph)
